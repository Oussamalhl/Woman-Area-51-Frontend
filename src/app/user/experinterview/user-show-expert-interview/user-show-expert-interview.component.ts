...
@Component({
  selector: 'app-user-show-expert-interview',
  templateUrl: './user-show-expert-interview.component.html',
  styleUrls: ['./user-show-expert-interview.component.scss']
})
export class UserShowExpertInterviewComponent implements OnInit {

  listInterviews:Expertinterview[]=[];
  dataSource = new MatTableDataSource<Expertinterview>(this.listInterviews);

  columnsToDisplay = ['dateexpertinterview','expertfield','interviewtype','rating','rateit','expert','Remove'];
  expint!:Expertinterview;
  public form!: FormGroup;

  constructor(private _service:ExpertinterviewService,private _liveAnnouncer: LiveAnnouncer) { }
  @ViewChild(MatSort) sort!: MatSort;
  @ViewChild(MatPaginator) paginator!: MatPaginator;

  ngOnInit(): void {
    this._service.getMyInterviews().subscribe(res=>{
      console.log(res);
